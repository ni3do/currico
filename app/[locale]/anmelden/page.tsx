...
function LoginPageContent() {
  ...
  // Auto-focus TOTP input when 2FA mode activates
  useEffect(() => {
    if (requires2FA) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Step 1: Check credentials and 2FA requirement
      ...
      const { requires2FA: needs2FA } = await checkRes.json();

      if (needs2FA) {
        // Show 2FA input — don't sign in yet
        setRequires2FA(true);
        return;
      }
            ...
            {requires2FA ? (
              ...
              <div className="animate-fade-in">
                <div className="mb-8 text-center">
                  ...
                  <h1 className="text-text text-2xl font-bold">{t("twoFactor.title")}</h1>
                  <p className="text-text-muted mt-2 text-sm">{t("twoFactor.description")}</p>
                </div>

                <form onSubmit={handle2FASubmit} className="space-y-6">
                  <div>
                    <label htmlFor="totp" className="text-text mb-2 block text-sm font-medium">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (useBackupCode) {
                          // Backup codes: alphanumeric, max 8
                          setTotpCode(val.slice(0, 8));
                        } else {
                      ...
                      className="border-border bg-surface text-text placeholder:text-text-muted focus:border-primary focus:ring-primary/20 w-full rounded-lg border px-4 py-3.5 text-center text-lg tracking-widest transition-all focus:ring-[3px] focus:outline-none"
                      placeholder={
                        useBackupCode
                  ...
                  <button
                    ...
                    className="text-primary hover:text-primary-hover w-full text-center text-sm font-medium transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="text-text-muted hover:text-text w-full text-center text-sm transition-colors"
                  >
                    {t("twoFactor.back")}
                  </button>
                </form>
              ...
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex items-center">
                    ...
                    <label
                      htmlFor="remember"
                      className="text-text-muted ml-2.5 cursor-pointer text-sm"
                    >
                      {t("form.rememberMe")}
                    </label>
                  </div>
